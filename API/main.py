...
from prediction import create_voting_regressor, prepare_data_generic, preprocess_features
import pandas as pd
...
async def create_dataframe(payload: dict, db: AsyncSession = Depends(get_db)):
    ...
    try:
        dataframe_croise = pd.merge(df_pays, df_table, on="id_pays", how="inner")  # `id_pays` est la clé de fusion
